    ...
    injectDataFirst: function initResponses(injectables, responses, wrap, hook) {
        return Object.keys(responses).reduce((acc, responseName) => {
            ...
            if(typeof responses[responseName] === 'object' && responses[responseName] !== null) {
                return Object.keys(responses).reduce((acc, subResponseName) => {
                    let response = (data) => {//it is assumed that responses have 1 external parameter from the user (function)
                        ...
                        if (typeof injectables.logger === 'function') {
                            LOG = new injectables.logger(subResponseName, hook);
                        }
                        this.injector(Object.assign({data: data}, injectables, {LOG: LOG, console: LOG}), responses[responseName][subResponseName]);
                    };
                    acc[subResponseName] = (typeof wrap === 'function') ? wrap(response) : response;
                });
            }
            else {
                let response = (data) => {//it is assumed that responses have 1 external parameter from the user (function)
                    ...
                    this.injector(Object.assign({data: data}, injectables, { LOG : LOG, console : LOG }), responses[responseName]);
                };
                acc[responseName] = (typeof wrap === 'function') ? wrap(response) : response;
            }
