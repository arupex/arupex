...
module.exports = function CodeGen(code, file){
    ...
    let obj2 = function (obj) {
        ...
        for (var p in obj) {
            if(p !== 'fncVarReplacements') {
                if (obj.hasOwnProperty(p)) {
                    if (typeof obj[p] === 'function') {
                        let funcStr = obj[p] + '';
                        /* jshint ignore:start */
                        if (obj.fncVarReplacements && obj.fncVarReplacements[p]) {
                            funcStr = Object.keys(obj.fncVarReplacements[p]).reduce((acc, v) => {
                                return acc.replace(v, '"' + obj.fncVarReplacements[p][v] + '"');
                            }, funcStr);
