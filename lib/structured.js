...
module.exports = {
     ...
     * @returns {*}
     */
    toStructure : function (object, firstParent, path) {
            ...
            return Object.keys(object).reduce((acc, key) => {
                if(key) {
                    ...
                    if (typeof object[key] === 'function') {
                        ...
                        this.extend(acc, this.toStructure(object[key], firstParent ? firstParent : key, (path ? path + '.' : '') + key));
                    }
                }
                return acc;
            }, {});
            ...
            return Object.keys(struct).reduce((acc, key) => {
                if (key) {
                    ...
                    if(key === 'data' && innerStructReference && !struct.error){
                        return asyncize.async(innerStructReference);
                    }
                    else if(key === 'error' && innerStructReference && !struct.data){
                        return asyncize.async(undefined, innerStructReference);
                    }
