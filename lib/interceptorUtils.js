...
class InterceptorUtils {
    ...
    constructor(injector, structured, clientBuilder, logger) {
    ...
    injectDataFirst(injectables, responses, wrap, hook) {
        const isWrapped = (typeof wrap === 'function');

        let createResponseFunc = (responseName, responseCode) => {
            return (data) => {//it is assumed that responses have 1 external parameter from the user (function)

                let LOG = new this._logger(responseName, hook);

                this._injector(Object.assign({data: data}, injectables, {
                    LOG: LOG,
                    console: LOG
                }), responseCode);
            };
        ...
        return Object.keys(responses).reduce((acc, responseName) => {
            // Support of Response object as a group of responses (so you can include a library of responses with 1 require file
            if (typeof responses[responseName] === 'object' && responses[responseName] !== null) {
                Object.keys(responses[responseName]).forEach((subResponseName) => {
                    let response = createResponseFunc(subResponseName, responses[responseName][subResponseName]);
                    acc[subResponseName] = isWrapped ? wrap(response) : response;
                });
            }
            else {
                let response = createResponseFunc(responseName, responses[responseName]);

                acc[responseName] = isWrapped ? wrap(response) : response;
            }
     ...
     * @returns {*}
     */
    aggregateInjector(initialInjectables, injectees, wrap, hook) {
        let injectables = initialInjectables;

        let last = {};
        injectees.forEach((injectee) => {
            last = this._injector(injectables, injectee, wrap, hook);
            injectables = Object.assign(injectables, last);
        });

        return Array.isArray(last) ? last : injectables;
    }


    /**
     * Removes Services from injectables
     * @param injectables
     * @param deinjectees
     * @returns {*}
     */
    aggregateDejector(injectables, deinjectees) {
        deinjectees.forEach((injectee) => {
            if (injectee && typeof injectee === 'object') {
                if (!Array.isArray(injectee)) {
                    Object.keys(injectee).forEach(key => {
    ...
    generatorReadyData(inputServices) {
     ...
     * @returns {*}
     */
    runtimeMockDataServices(useableDataServices, mockContext) {
        ...
        Object.keys(useableDataServices).forEach((serviceName) => {
            if (typeof useableDataServices[serviceName].overrideable !== 'boolean' || useableDataServices[serviceName].overrideable) {
                useableDataServices[serviceName] = this._structured.toImplentation(mockContext[serviceName]);
     ...
     */
    setupWorkers(workers, injectables) {
            ...
            Object.keys(workers).forEach(worker => {
                if (typeof workers[worker] === 'function') {
                    workerInstances[worker] = setInterval(() => {
                        this._injector(injectables, workers[worker]);
     ...
     * @param warn
     */
    fillEmpty(app, names, defaults, warn) {
        names.forEach((name) => {
     ...
     * @returns {*}
     */
    clientBuild(nonInjectableServices, activeEnvironment, loggerHooks) {
        if (loggerHooks === null || typeof loggerHooks !== 'object') {
            loggerHooks = {};
        }

        const internalLogger = new this._logger('interceptor-utils', {level: activeEnvironment.logLevel || 'crucial'});
        ...
        if (typeof nonInjectableServices === 'object') {
            return Object.keys(nonInjectableServices).reduce((acc, v) => {
                ...
                if (serviceExistsInEnvironmentConfig &&
                    ...
                    activeEnvironment[v].logger = new this._logger(v, activeLoggerHook).info;//has to be a function for hyper-request to work
                    acc[v] = this._clientBuilder(nonInjectableServices[v])._init(activeEnvironment[v]);
                }
                else {
                    if (!serviceExistsInEnvironmentConfig) {
