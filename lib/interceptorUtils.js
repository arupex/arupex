    ...
    clientBuild: function calculateInjectableDataServices(nonInjectableServices, activeEnvironment, loggerHooks) {
        ...
        if (typeof nonInjectableServices === 'object') {
                return Object.keys(nonInjectableServices).reduce((acc, v) => {
                    ...
                    const serviceIsLikelyAnArupexClient = serviceIsANonNullObject && Array.isArray(nonInjectableServices[v]) && nonInjectableServices[v].every(elementIsObject);

                    internalLogger.info(`configuring ${v}:
                        serviceExistsInEnvironmentConfig=${serviceExistsInEnvironmentConfig}
                        serviceIsANonNullObject=${serviceIsANonNullObject}
                        serviceIsLikelyAnArupexClient=${serviceIsLikelyAnArupexClient}
                        ...
                    `);
                    ...
                    if(serviceExistsInEnvironmentConfig &&
                        ...
                        internalLogger.info(`${v} is being initalized as an arupex client...`);
