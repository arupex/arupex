...
module.exports = function ClientBuilder (calls, injectables) {
    ...
    client.clearInternalCache = function(){
        this.restClient.clearCache();
    };

    client.setLogger = function(logger){
        this._log = logger;
    };
    ...
    calls.forEach( c => {
        ...
        client[c.name] = async function (opts, ok, fail) {
            if(!this.initialized){
                this.init(opts);
            }
            ...
            if(typeof ok === 'function' || typeof fail === 'function') {
                this._log.error('Arupex clientBuilder no longer supports callback functions, client methods are async');
            }

            if(typeof this.restClient[c.method.toLowerCase()] === 'function') {
                return this.restClient[c.method.toLowerCase()](url, opts);
            }
            else {
                this._log.error(`${c.method.toLowerCase()} is not a method`);
                throw new Error(`${c.method.toLowerCase()} is not a method`);
            }
        };
