...
class DocGenerator {
    ...
    constructor(routeAnalyzer) {
        this._routeAnalyzer = routeAnalyzer;
    }

    swaggerRoute(method, path, pathParams, query, headers){
    ...
    generateFromUrls(urls, opts){
        let decompiledRoute = this._routeAnalyzer.decompileRoute;
        if(!opts) {
            opts = {};
        }
