...
class RouteAnalyzer {
    ...
    decompileRoute(route) {
        ...
        const path = decodeURIComponent(routeUrl);//because url.parse encodes it :(
        let method = 'GET';
    ...
    conductor(routeMap) {
        ...
        return (requestedRoute) => {
            ...
            let routesFound = routes.filter( route => {
                if (route.regex.test(requestedRoute)) {
                    LOG.info(`matched path: ${route.path}`);
                    return true;
                }
            });
    ...
    findLongestRoute(routesFound) {
        ...
        if (routesFound.length > 1) {
            LOG.warn(`multiple paths matched looking for the one with the largest length`);
            ...
            if (routeLengths.count > 1) {
                LOG.warn(`multiple paths were considered the 'longest', selecting the last one`);
            }
            routeFind = routeLengths.last;
            LOG.warn(`selected path: ${routeFind.path}`);
        }
