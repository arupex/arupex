...
class RouteAnalyzer {
    ...
    constructor(logger) {
        this._LOG = new logger('Arupex-RouteAnalyzer');
    }
    ...
    findLongestRoute(routesFound) {
        ...
        if (routesFound.length > 1) {
            this._LOG.warn(`multiple routes matched looking for the one with the largest length`);
            ...
            if (routeLengths.count > 1) {
                this._LOG.warn(`multiple routes were considered the 'longest', selecting the last one`);
            }
