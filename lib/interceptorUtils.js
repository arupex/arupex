...
module.exports = {
    ...
    injectDataFirst: function initResponses(injectables, responses, wrap, hook) {
        ...
        const loggerExists = typeof injectables.logger === 'function';
        ...
        let createResponseFunc = (responseName, responseCode) => {
            return (data) => {//it is assumed that responses have 1 external parameter from the user (function)
                ...
                if (loggerExists) {
                    LOG = new injectables.logger(responseName, hook);
                }
                this.injector(Object.assign({data: data}, injectables, {
                    LOG: LOG,
                    console: LOG
                }), responseCode);
            };
     ...
     * @returns {*}
     */
    aggregateDejector: function aggregateDejector(injectables, deinjectees) {
    ...
    generatorReadyData: function generatorReadyData(inputServices) {
        return Object.keys(inputServices).reduce((services, serviceName) => {
            services[serviceName] = Object.keys(inputServices[serviceName]).reduce((service, funcName) => {
                if (['restClient', 'init', 'clearInternalCache', 'interpolate', 'debug', 'initialized', 'fncVarReplacements', 'setLogger', '_log'].indexOf(funcName) === -1) {
    ...
    clientBuild: function calculateInjectableDataServices(nonInjectableServices, activeEnvironment, loggerHooks) {
        ...
        const internalLogger = new this.logger('Arupex-Interceptor-Utils', { level : activeEnvironment.logLevel || 'crucial' });
